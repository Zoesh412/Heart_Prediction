...
# ============ 4. 构建GRU模型 ============
def create_gru_single(input_shape):
    """单步预测GRU"""
    model = Sequential([
...
def create_gru_multi(input_shape, output_size=1500):
    """多步预测GRU"""
    model = Sequential([
...
# ============ 6. PPT风格的单步预测图 ============
def plot_ppt_style_single(model, X_test, y_test, save_name='GRU_单步预测结果.png'):
    """
    画单步预测的实际值 vs 预测值对比图
    """
    print("\n" + "="*50)
    ...
    plt.plot(x_axis, y_pred_all, 'r--', label='Predicted (GRU)', linewidth=1.5, alpha=0.8)
    
    plt.title('GRU Single-Step Prediction (20→1)')
    ...
    plt.show()
    print(f"GRU单步预测图已保存为：{save_name}")
    
    # 计算并打印MAE
    mae = np.mean(np.abs(y_pred_all - y_true_all))
    print(f"GRU单步预测MAE: {mae:.4f}秒")
    
    return y_true_all, y_pred_all

# ============ 7. PPT风格的多步预测对比图 ============
def plot_ppt_style_multistep(model, X_test, y_test, train_mean, train_std, save_name='GRU_多步预测结果.png'):
    """
    画多步预测的实际值 vs 预测值对比图
    """
    print("\n" + "="*50)
    ...
    plt.plot(x_axis, y_pred[:steps], 'r--', label='Predicted (GRU)', linewidth=1.5, alpha=0.8)
    
    plt.title('GRU Multi-Step Prediction (1500→1500)')
    ...
    plt.savefig(save_name, dpi=150, bbox_inches='tight')
    plt.show()
    print(f"GRU多步预测图已保存为：{save_name}")
...
def main():
    ...
    model_single = create_gru_single((20, 1))
    ...
    plot_ppt_style_single(
        ...
        X_test_single_3d,
        y_test_single,
        save_name='GRU_单步预测结果.png'
    )
    
    # ===== 多步预测 =====
    print("\n" + "="*60)
    print("GRU多步预测 (输入1500个，输出1500个)")
    print("="*60)
    ...
    if len(X_train_multi) > 0 and len(X_test_multi) > 0:
        ...
        # 创建多步预测模型
        model_multi = create_gru_multi((1500, 1), 1500)
        model_multi.summary()
        ...
        if multi_result:
            plot_ppt_style_multistep(
                ...
                train_mean,
                train_std,
                save_name='GRU_多步预测结果.png'
            )
    
    print("\n" + "="*60)
    print("GRU模型测试完成！")
    print("生成的文件：")
    print("  1. GRU_单步预测结果.png - 单步预测对比图")
    print("  2. GRU_多步预测结果.png - 多步预测对比图")
    print("="*60)
