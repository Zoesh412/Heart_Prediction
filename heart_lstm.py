...
# ============ 4. 构建LSTM模型 ============
def create_lstm_single(input_shape):
    """单步预测LSTM"""
    model = Sequential([
...
def create_lstm_multi(input_shape, output_size=1500):
    """多步预测LSTM"""
    model = Sequential([
...
# ============ 6. PPT风格的单步预测图 ============
def plot_ppt_style_single(model, X_test, y_test, save_name='PPT风格单步预测结果.png'):
    """
    画单步预测的实际值 vs 预测值对比图
    """
    print("\n" + "="*50)
    print("生成PPT风格的单步预测对比图")
    print("="*50)
    ...
    plt.plot(x_axis, y_pred_all, 'r--', label='Predicted', linewidth=1.5, alpha=0.8)
    
    plt.title('LSTM Single-Step Prediction (20→1)')
    ...
    plt.show()
    print(f"PPT风格单步预测图已保存为：{save_name}")
    
    # 计算并打印MAE
    mae = np.mean(np.abs(y_pred_all - y_true_all))
    print(f"单步预测MAE: {mae:.4f}秒")
    
    return y_true_all, y_pred_all

# ============ 7. PPT风格的多步预测对比图 ============
def plot_ppt_style_multistep(model, X_test, y_test, train_mean, train_std, save_name='PPT风格多步预测结果.png'):
    """
    画多步预测的实际值 vs 预测值对比图（和PPT一模一样）
    """
    print("\n" + "="*50)
    print("生成PPT风格的多步预测对比图")
    print("="*50)
    ...
    plt.plot(x_axis, y_true[:steps], 'b-', label='Actual Last 1500', linewidth=1.5, alpha=0.8)
    plt.plot(x_axis, y_pred[:steps], 'r--', label='Predicted Last 1500', linewidth=1.5, alpha=0.8)
    
    plt.title('LSTM Multi-Step Prediction (1500→1500)')
    ...
    plt.savefig(save_name, dpi=150, bbox_inches='tight')
    plt.show()
    print(f"PPT风格多步预测图已保存为：{save_name}")
...
# ============ 8. 可视化结果 ============
def plot_results(clean_data, single_result, multi_result):
    """可视化结果"""
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    
    # 1. 数据分布
    axes[0,0].hist(clean_data, bins=50, alpha=0.7, color='blue')
    axes[0,0].set_title('数据分布')
    axes[0,0].set_xlabel('心跳间隔（秒）')
    axes[0,0].set_ylabel('频次')
    axes[0,0].axvline(0.3, color='r', linestyle='--', label='下界0.3秒')
    axes[0,0].axvline(2.0, color='r', linestyle='--', label='上界2.0秒')
    axes[0,0].legend()
    
    # 2. 时间序列
    axes[0,1].plot(clean_data[:500], linewidth=0.8)
    axes[0,1].set_title('心跳间隔时间序列（前500个）')
    axes[0,1].set_xlabel('心跳次数')
    axes[0,1].set_ylabel('间隔时间（秒）')
    
    # 3. MAE对比
    names = [single_result['model_name'], multi_result['model_name']]
    maes = [single_result['mae'], multi_result['mae']]
    colors = ['green', 'orange']
    bars = axes[1,0].bar(names, maes, color=colors)
    axes[1,0].set_title('模型MAE对比')
    axes[1,0].set_ylabel('MAE（秒）')
    ...
    # 4. 训练损失
    if single_result and 'history' in single_result:
        axes[1,1].plot(single_result['history'].history['loss'], 
                      label=f"{single_result['model_name']}-训练")
        axes[1,1].plot(single_result['history'].history['val_loss'], '--', 
                      label=f"{single_result['model_name']}-验证")
    if multi_result and 'history' in multi_result:
        axes[1,1].plot(multi_result['history'].history['loss'], 
                      label=f"{multi_result['model_name']}-训练")
        axes[1,1].plot(multi_result['history'].history['val_loss'], '--', 
                      label=f"{multi_result['model_name']}-验证")
    ...
    plt.tight_layout()
    plt.savefig('心跳预测结果.png', dpi=150, bbox_inches='tight')
    plt.show()
    print("\n图表已保存为：心跳预测结果.png")
...
def main():
    ...
    plot_ppt_style_single(
        ...
        X_test_single_3d,
        y_test_single,
        save_name='PPT风格单步预测结果.png'
    )
    
    # ===== 多步预测 =====
    print("\n" + "="*60)
    print("多步预测 (输入1500个，输出1500个)")
    print("="*60)
    ...
    if len(X_train_multi) > 0 and len(X_test_multi) > 0:
        ...
        # 创建多步预测模型
        model_multi = create_lstm_multi((1500, 1), 1500)
        model_multi.summary()
    ...
    print("\n| 模型 | MAE(秒) | 10次推理(秒) | 训练时间(秒) |")
    print("|------|---------|--------------|--------------|")
    print(f"| {single_result['model_name']} | {single_result['mae']:.4f} | {single_result['inference_time_10']:.2f} | {single_result['train_time']:.2f} |")
    if multi_result:
        print(f"| {multi_result['model_name']} | {multi_result['mae']:.4f} | {multi_result['inference_time_10']:.2f} | {multi_result['train_time']:.2f} |")
    ...
    print(f"\n单步LSTM (20-1)对比：")
    print(f"  PPT结果  - MAE: 0.0117, 10次推理: 0.25秒")
    print(f"  本实验   - MAE: {single_result['mae']:.4f}, 10次推理: {single_result['inference_time_10']:.2f}秒")
    print(f"  差异     - MAE: {single_result['mae']-0.0117:+.4f}, 推理时间: {single_result['inference_time_10']-0.25:+.2f}秒")
    
    if multi_result:
        print(f"\n多步LSTM (1500-1500)对比：")
        print(f"  PPT结果  - MAE: 0.0179, 10次推理: 0.11秒")
        print(f"  本实验   - MAE: {multi_result['mae']:.4f}, 10次推理: {multi_result['inference_time_10']:.2f}秒")
        print(f"  差异     - MAE: {multi_result['mae']-0.0179:+.4f}, 推理时间: {multi_result['inference_time_10']-0.11:+.2f}秒")
    
    # 可视化
    if multi_result:
        plot_results(clean_data, single_result, multi_result)
    
    print("\n" + "="*60)
    print("程序运行完成！")
    print("生成的文件：")
    print("  1. 心跳预测结果.png - 训练过程统计图")
    print("  2. PPT风格单步预测结果.png - 单步预测对比图")
    print("  3. PPT风格多步预测结果.png - 多步预测对比图（和PPT一样）")
    print("="*60)
